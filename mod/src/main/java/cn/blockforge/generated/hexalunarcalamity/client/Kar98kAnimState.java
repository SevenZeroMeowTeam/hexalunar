...
/**
 * Kar98k 的客户端便捷查询（骨骼动画控制器 + {@link Kar98kGeoModel} 共用）。
 *
 * <p>与 {@link AwpAnimState} 同一套：动作进度**不在这里维护**，换弹 / 拉栓 / 击发的窗口
 * 全部读物品 NBT（{@code HlcReloadUntil} / {@code HlcBoltUntil} / {@code HlcFireAt}），
 * 服务端写、客户端读同一份，永远不会出现「动画演完了动作还没结束」的错位。
 * 这里只负责「谁拿着枪」这类本地状态。
 */
public final class Kar98kAnimState {

    private Kar98kAnimState() {
    }
    ...
    /** 本地玩家主手的 Kar98k（不是它返回 null） */
    public static ItemStack heldStack() {
        Player player = Minecraft.getInstance().player;
        if (player == null) return null;
        ItemStack stack = player.getMainHandItem();
        return stack.getItem() instanceof Kar98kItem ? stack : null;
    }
    ...
    /** 正按住右键抵肩瞄准（正在 use 这把 Kar98k） */
    public static boolean aiming() {
        Player player = Minecraft.getInstance().player;
        if (player == null || !player.isUsingItem()) return false;
        return player.getUseItem().getItem() instanceof Kar98kItem;
    }
}
