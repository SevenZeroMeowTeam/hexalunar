...
/**
 * Kar98k 的 GeckoLib 物品渲染器（挂在物品的 {@code initializeClient} 上，
 * 所以物品本身与 GUI 图标全部由骨骼模型渲染）——照 {@link AwpGeoRenderer} 抄。
 */
public class Kar98kGeoRenderer extends GeoItemRenderer<Kar98kItem> {

    private static Kar98kGeoRenderer instance;

    /** 会动的光泽层：抛光高光处自发光 + 开火/举枪时更亮（遮罩由 kar98k_gen.py 一并生成） */
    private final GlossGlintLayer<Kar98kItem> glint;

    public Kar98kGeoRenderer() {
        super(new Kar98kGeoModel());
        this.glint = new GlossGlintLayer<>(this, GlossGlintLayer.mask("kar98k_geo"), 0.50F,
                WeaponAnim.Kind.KAR98K);
    }
    ...
    public void renderByItem(net.minecraft.world.item.ItemStack stack,
        ...
        Kar98kGeoModel.handPass = isHand(transformType);
        try {
            super.renderByItem(stack, transformType, poseStack, bufferSource, packedLight, packedOverlay);
        } finally {
            Kar98kGeoModel.handPass = false;
        }
    }
    ...
    @Override
    public List<GeoRenderLayer<Kar98kItem>> getRenderLayers() {
        return List.of(glint);
    }

    public static Kar98kGeoRenderer get() {
        if (instance == null) {
            instance = new Kar98kGeoRenderer();
        }
        return instance;
    }
}
