...
- 模组 ID：`hexalunar_calamity`｜版本：`1.0.0-r63`
...
| 产物 | `mod/build/libs/hexalunar_calamity-1.0.0-r63.jar` |
...
> **十字弩的弓臂（r63）**：拉弦时两弓臂**向内收缩** —— 绕「贴导轨的内端」向内转（拉满 **9°**），
> 外端因此往**内 0.55 + 后 0.31**（模型像素）走，弓臂外缘宽度 7.06 → 6.24（**收 12%**）；
> 弦与凸轮盘的 pivot 就在弓臂梢上，所以它们跟着弓臂一起走，弦不会脱开。
> **击发/未拉弦时回到参考网格（图片）那个张开姿态**（`draw = 0` ⇒ 转角 0）。
> 离线验算 + 出图：`python tools/_cb_flex.py [--bake 1.0]`（会打印收进量与弦内端偏差，
> 烘焙出来的 `build/cb_flex_*.geo.json` 可以直接丢给 `geo_texview.py` 出对比图）。
...
- **枪稳在手里、开火时双手跟着一起顿**（r59）：
  ...
  - **手臂与枪的同步靠 frame 捕获（r63）**：`WeaponArms` 是在 `RenderHandEvent` 里、**比物品先画**，
    读的是上一帧的捕获；图标那一遍（display=GUI）如果也去写 frame，手臂拿到的就是「无举枪、无后坐」
    的空值 ⇒ 看上去就是**枪在动、手不跟着（上下错位）**。现在 frame **只在手持语境下更新**
...
| **弩弓臂内收角 / 弯折支点** | `client/CrossbowGeoModel.java` 的 `FLEX_DEG` / `FLEX_PX` / `FLEX_PZ`（拉弦时弓臂绕「贴导轨内端」向内转）；**必须**与 `tools/_cb_flex.py` 的同名常数一致，改完跑一遍那个脚本看收进量与弦内端偏差 |
